...
public class PlayAnalyzeUI extends ServerSideUI {
	...
	public PlayAnalyzeUI() {
		Rectangle rect = contPan.getBounds();

		SaleAnalyzeSrv saleAnalyze = new SaleAnalyzeSrv();
		List<Play> playList = saleAnalyze.getAllPlay();
		// 构建容器
		contain = new JPanel(new GridLayout(playList.size(), 1));
		contain.setBounds(0, 200, rect.width, 400);

		// 添加头部
		header = new JLabel("售票分析(剧目)", JLabel.CENTER);
		header.setBounds(0, 10, rect.width, 30);
		header.setFont(new java.awt.Font("宋体", 1, 20));
		header.setForeground(Color.yellow);
		contPan.add(header);

		header = new JLabel("剧目名称", JLabel.CENTER);
		header.setBounds(0, 30, rect.width / 2, 30);
		header.setFont(new java.awt.Font("宋体", 1, 20));
		header.setForeground(Color.yellow);
		contPan.add(header);

		header = new JLabel("剧目销售额", JLabel.CENTER);
		...
		for (Play s : playList) {
			...
			int sale_play = saleAnalyze.addByPlay(s.getPlay_id());
			textName = new JLabel(s.getPlay_name(), JLabel.CENTER);
